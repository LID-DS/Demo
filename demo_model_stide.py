...
class DemoModelStide:
    def __init__(self, ngram_length=7, window_length=1000, training_size=10000000, trained_model=None):
        """
    ...
    def _get_syscall_as_int(self, syscall_string):
        ...
        :return: an integer representing the given system call regarding this model instance
        """
    ...
    def _train_on(self, ngram_tuple):
        ...
        if the number of system call seen  is bigger than self._training_size it will switch
        to detection mode
    ...
    def _detect(self, ngram_tuple):
        """
        checks the given tuple against the normal db and calculates the anomaly score
        :param ngram_tuple:
        ...
        # 0 for no, 1 for yes
        right_value = 1 if (self._normal_ngrams.get(ngram_tuple) is None) else 0
    ...
    def _ngram_tuple_to_str(self, ngram_tuple):
        """
        converts a tuple of ints to a readable string eg: (1,4,2) -> "open read close"
